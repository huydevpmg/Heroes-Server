...
export const updateHero = async (req, res) => {
  try {
    const updated = await HeroService.updateHeroService(
      req.params.id,
      req.user.id,
      req.body
    );
...
export const deleteHero = async (req, res) => {
  try {
    const deleted = await HeroService.deleteHeroService(
      req.params.id,
      req.user.id
    );
    if (!deleted) {
...
export const updateHeroTags = async (req, res) => {
  try {
    const updated = await HeroService.updateHeroTagsService(
      req.params.id,
      req.user.id,
      req.body.tags
    );
...
export const bulkAddTagController = async (req, res) => {
  ...
  if (!Array.isArray(heroIds) || !tag) {
    return res
      .status(400)
      .json({ message: "heroIds (array) and tag are required" });
  }

  try {
    const result = await HeroService.bulkAddSingleTagToHeroesService(
      heroIds,
      tag
    );
...
export const bulkRemoveTagController = async (req, res) => {
  ...
  try {
    const result = await HeroService.bulkRemoveSingleTagFromHeroesService(
      heroIds,
      tag
    );
    res.status(200).json({ heroIds, tag });
